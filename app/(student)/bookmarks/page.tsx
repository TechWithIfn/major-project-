      ...
      <div className="flex flex-col gap-5 sm:gap-6">
        <div className="relative group">
          ...
          <Input
            ...
            className="pl-11 h-12 rounded-2xl premium-glass border-border/50 focus:ring-primary/20"
            value={searchQuery}
        ...
        {items.length === 0 ? (
          <Card className="p-12 text-center glass-card premium-glass border-dashed">
            ...
            <h2 className="type-premium-title mt-6 text-xl text-foreground">No Knowledge Saved Yet</h2>
            <p className="type-premium-body mt-2 text-sm text-muted-foreground max-w-sm mx-auto">Start a conversation with ShikshaSahayak or generate a quiz to build your personal knowledge repository here.</p>
          </Card>
        ) : filteredItems.length === 0 ? (
          <div className="p-12 text-center">
            <p className="type-premium-body text-muted-foreground font-medium">No results found for &quot;{searchQuery}&quot;</p>
          </div>
        ) : (
          <div className="grid gap-6">
            {filteredItems.map((item) => (
              <Card key={item.id} className="glass-card premium-glass overflow-hidden group hover:border-primary/30 transition-all duration-300">
                <div className="flex items-start justify-between p-6 bg-muted/30 border-b border-border/40">
                  <div className="flex items-center gap-4 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <h3 className="type-premium-title text-foreground text-base text-clamp-2 break-words">{item.title}</h3>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-8 w-8 rounded-full text-muted-foreground hover:text-destructive hover:bg-destructive/10 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all"
                ...
                <div className="p-6">
                  <p className="type-premium-body whitespace-pre-wrap text-[15px] text-foreground/80 font-medium">{item.content}</p>
                </div>
              </Card>
            ))}
          </div>

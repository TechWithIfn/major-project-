...
import { useProgressDashboard } from '../hooks/use-offline-study';
            ...
            {data.subjectSummaries.map((summary) => (
              <section key={summary.subjectId} className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                ...
                <div className="mt-2 h-2 rounded-full bg-slate-200 dark:bg-slate-800">
                  <div className="h-full rounded-full bg-teal-600" style={{ width: `${summary.completionRate}%` }} />
                </div>
                <ul className="mt-3 space-y-2">
                  {summary.chapters.map((chapter) => (
                    <li key={chapter.chapterId} className="flex items-center justify-between gap-2 rounded-lg bg-slate-50 px-3 py-2 text-sm dark:bg-slate-800/70">
                      <span className="inline-flex items-center gap-2">
                        {chapter.completed ? <CheckCircle2 size={16} className="text-emerald-600" /> : <Circle size={16} className="text-slate-400" />}
                        {chapter.title}
                      </span>
                      <span className="text-xs text-slate-500 dark:text-slate-400">{chapter.lastPosition > 0 ? `Last position ${chapter.lastPosition}px` : 'Not started'}</span>
                    </li>
                  ))}
                </ul>
              </section>
